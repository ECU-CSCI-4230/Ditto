...
//    $file_ext=strtolower(end(explode('.',$_FILES['image']['name'])));
    ...
    if (file_exists("uploads/$file_name")) {
        echo '<script>display_error("' . $file_name . ' A file with this name exists already.");</script>';
        $err = 1;
    }else if($file_size > 8388608){
        echo '<script>display_error("' . $file_name . ' is ' . $file_size/1048576 . ' Mb... Max size is 8 Mb");</script>';
        $err = 1;
    }else if ($err == 0) {
